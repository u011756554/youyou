...
//        System.out.println("压缩测试   文件图片压缩： 采样率 "+be+" 质量压缩比例 "+compressOption);
	...
	private String preparePicturePathForJPG() {
		String filename = YouYouTools.instance().getFormedTime(System.currentTimeMillis()) + ".jpg";
	...
	private String preparePicturePathForPNG() {
		String filename = YouYouTools.instance().getFormedTime(System.currentTimeMillis()) + ".png";
		String path = Environment.getExternalStorageDirectory().getAbsolutePath() + 
				AppContext.SAVE_PATH;
		File pathFile = new File(path);
		if(!pathFile.exists()) {
