...
public class ImageLoaderUtil {
	...
	public static DisplayImageOptions resetFadeInOptions = new DisplayImageOptions.Builder().cacheInMemory(true).cacheOnDisk(true).considerExifParams(true)
			.displayer(new FadeInBitmapDisplayer(0)).bitmapConfig(Bitmap.Config.RGB_565).imageScaleType(ImageScaleType.EXACTLY)
			.build();
	...
	public static void disPlayByImage(final String uri, final ImageView imageView) {
		...
		new AsyncTask<Void, Void, Bitmap>() {
			...
			protected void onPostExecute(Bitmap result) {
				if (result == null)
					return;
				imageView.getLayoutParams().width = CommonUtil.dip2px(imageView.getContext(), result.getWidth() / 2);
				imageView.getLayoutParams().height = CommonUtil.dip2px(imageView.getContext(), result.getHeight() / 2);
